...
async function testConnection() {
  ...
  const { data: userState, error: userStateErr } = await supabase.from('user_cloud_state').select('*').limit(1);
  if (userStateErr) {
    console.log("Error querying user_cloud_state:", userStateErr.message);
  } else {
